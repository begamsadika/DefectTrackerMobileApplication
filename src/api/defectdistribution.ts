...
export const getDefectDistributionByType = async (projectId: number | string) => {
    try {
      const response = await axios.get(`http://192.168.43.135:3000/api/dashboard/defect-distribution-by-type/${projectId}`);
      console.log(`Defect Distribution By Type Response for Project ${projectId}:`, response.data);
      console.log('Success:', response.data.status || 'Request successful');
